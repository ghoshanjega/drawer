...
public class RectangleCommand extends Command {
    ...
    RectangleCommand(String[] command) throws InvalidCommandException, IllegalArgumentException{
        super('R');
        if (command.length < COMMAND_COUNT) throw  new InvalidCommandException("Expected format => R [x1] [y1] [x2] [y2]");
